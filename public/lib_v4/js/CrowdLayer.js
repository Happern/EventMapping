function CrowdLayer(map) 
{
	...
	{
		this.dataFinal.push({type:"Feature", geometry:{type:"Point", coordinates:[this.data[i][1], this.data[i][0]]}});
		for(j=0; j<20; j++)
		{
			this.dataFinal.push({type:"Feature", geometry:{type:"Point", coordinates:this.calcRandom(this.data[i], 0.010)}});
		}

		for(j=0; j<10; j++)
			this.dataFinal.push({type:"Feature", geometry:{type:"Point", coordinates:this.calcRandom(this.data[i], 0.005)}});

	}
	
	for(i=0; i<this.data.length; i++)
	{
		this.dataFinal2.push({type:"Feature", geometry:{type:"Point", coordinates:[this.data[i][1], this.data[i][0]]}});
		for(j=0; j<40; j++)
			this.dataFinal2.push({type:"Feature", geometry:{type:"Point", coordinates:this.calcRandom(this.data[i], 0.002)}});
/*
		for(j=0; j<40; j++)
			this.dataFinal2.push({type:"Feature", geometry:{type:"Point", coordinates:[this.data[i][1]+Math.random()*0.050-0.0025, 
				this.data[i][0]+Math.random()*0.050-0.0025]}});
*/
	}
	...
	this.layer ={
        ...
        'paint': {
            ...
            'circle-radius': {
                'base': 1,
                'stops': [[2, 1.0], [12, 1.0], [13, 1.5], [22, 1.5]]
            },
	...
	this.map.setLayoutProperty("crowd", 'visibility', 'none');
	this.map.setLayoutProperty("crowd2", 'visibility', 'none');
}
...
{
	this.map.setLayoutProperty("crowd", 'visibility', 'visible');
	if(this.currentLayerId)
	this.map.setLayoutProperty(this.currentLayerId, 'visibility', 'visible');
	this.visible=true;
}
CrowdLayer.prototype.hide=function()
{
	this.map.setLayoutProperty("crowd", 'visibility', 'none');
	if(this.currentLayerId)
	this.map.setLayoutProperty(this.currentLayerId, 'visibility', 'none');
	this.visible=false;
}

CrowdLayer.prototype.update=function(zoomValue)
{
	//console.log(zoomValue);
	if(zoomValue>13)
	{
		this.currentLayerId="crowd2";
		if(this.visible)
		this.map.setLayoutProperty(this.currentLayerId, 'visibility', 'visible');
	}
	else
...
CrowdLayer.prototype.calcRandom=function(center, maxD)
{
	var r=maxD*Math.random();
	var a=Math.random()*Math.PI*2;
	var x=r*Math.cos(a);
	var y=r*Math.sin(a);
	var newCoord=[center[1]+x, center[0]+y];
	//console.log(center, newCoord);
	return newCoord;
}