...
EventLayer.prototype.getIconLevel0=function(item)
{
	...
	var template = [
		...
		'<path d="{{ path }}" stroke="none"  fill="{{ color }}" fill-opacity="0.75"/>',
		'</g>',
		'</g>',
		'</svg>'
	...
	return { url: 'data:image/svg+xml;charset=UTF-8;base64,' + btoa(svg), scaledSize:50 };
}
...
EventLayer.prototype.getIconLevel2=function(item)
{
	...
	svg = svg.replace('{{ r1 }}', r1);
	 ...
	 //return { url: 'data:image/svg+xml;charset=UTF-8,' + encodeURIComponent(svg), scaledSize: new google.maps.Size(20, 20) };
	 return { url: 'data:image/svg+xml;charset=UTF-8;base64,' + btoa(svg), scaledSize:70};
}
...
{
	var type=new Array();
	$("#filterEventType .filter-option.has-selected").each(function(i, item){
		type.push($(item).attr("data-value"));
		});
		
	var price=new Array();
	$("#filterEventPrice .filter-pricelist-item.has-selected").each(function(i, item){
		price.push($(item).attr("data-value"));
		});		
		
	var attending=new Array();
	$("#filterEventAttending .filter-radiallist-item.radiallist-item-selected").each(function(i, item){
		attending.push($(item).attr("data-value"));
		});
		
	var venue=new Array();
	$("#filterEventVenue .filter-corneredlist-item.corneredlist-item-selected").each(function(i, item){
		venue.push($(item).attr("data-value"));
		});
		
	var sound=new Array();
	$("#filterEventSound .filter-radiallist2-item.radiallist2-item-selected").each(function(i, item){
		sound.push($(item).attr("data-value"));
		});		
		
	var activity=new Array();
	$("#filterEventActivity .filter-radiallist3-item.radiallist3-item-selected").each(function(i, item){
		activity.push($(item).attr("data-value"));
		});
	return {
...
EventLayer.prototype.updateMarkers=function()
{
	...
	for(i=0; i<this.list.length; i++)
	{
		...
		
		{
			...
			width: icon.scaledSize,
			height: icon.scaledSize,
