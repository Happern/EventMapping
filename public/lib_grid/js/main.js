...
function gotInitData(data)
{
	...
	//	console.log(data.twitter[i].lat,",", data.twitter[i].lng);
	...
	_GLOBAL.traffic=new TrafficLayer(_GLOBAL.map);
	_GLOBAL.traffic.init();
	...
	roads.init();
*/
	google.maps.event.addListener(_GLOBAL.map, 'zoom_changed', zoomChanged);
...
{
	if(value)
		_GLOBAL.traffic.show();
	else
		_GLOBAL.traffic.hide();
}
...
{
	var zoom=_GLOBAL.map.getZoom();
	var changed=false;
	console.log(zoom);
	if(zoom<=12)
	{
		if(_GLOBAL.event.mapLevel!=0)
		{
			_GLOBAL.event.mapLevel=0;
			changed=true;
		}
	}
	else if(zoom<=14)
	{
		if(_GLOBAL.event.mapLevel!=1)
		{
			_GLOBAL.event.mapLevel=1;
			changed=true;
		}
	}
	else
	{
		if(_GLOBAL.event.mapLevel!=2)
		{
			_GLOBAL.event.mapLevel=2;
			changed=true;
		}
	}
	if(changed)
		_GLOBAL.event.updateMarkers();
}
